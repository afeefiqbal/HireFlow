...
class ScreeningService {
     ...
     */
    static async analyzeScreeningQuestions(jobId, rawQuestions) {
        let questionsList = [];
        if (!rawQuestions) {
            // Look up in job
        ...
        // ---------------------------------------------------------
        const llmResults = await ai_service_1.AiService.answerScreeningQuestions(jobId, questionsList, profile, isExplicitRequest);
        const results = [];
        for (const res of llmResults) {
            const q = res.question;
            const requiresUserInput = res.requiresUserInput;
            const suggestedAnswer = res.suggestedAnswer;
            let confidence = requiresUserInput ? 'low' : 'high';
    ...
    static async answerScreeningQuestion(id, answer) {
