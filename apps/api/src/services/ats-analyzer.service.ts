...
export class AtsAnalyzerService {
   ...
   */
  static async analyzeResume(resumeVersionId: string, jobId: string): Promise<AtsAnalysisResult> {
    const job = await prisma.job.findUnique({ where: { id: jobId } });
    if (!job) throw new Error(`Job not found: ${jobId}`);

    const resumeVer = await prisma.resumeVersion.findUnique({ where: { id: resumeVersionId } });
    if (!resumeVer) throw new Error(`ResumeVersion not found: ${resumeVersionId}`);

    const cvData = resumeVer.contentJson as unknown as TailoredCvData;
    ...
    const requiredMatches: AtsSkillMatchItem[] = [];
    ...
    const preferredMatches: AtsSkillMatchItem[] = [];
    ...
    if (jobTitleLower.includes('laravel') && cvTitleLower.includes('laravel')) titleAlignment = 95;
    else if (jobTitleLower.includes('node') && cvTitleLower.includes('node')) titleAlignment = 95;
    else if (jobTitleLower.includes('full stack') && cvTitleLower.includes('full stack')) titleAlignment = 95;
    else if (jobTitleLower.includes('php') && cvTitleLower.includes('php')) titleAlignment = 90;
    ...
    const overallCoverage = Math.round(
      requiredCoverage * 0.55 + preferredCoverage * 0.25 + titleAlignment * 0.1 + experienceAlignment * 0.1
    );
    ...
    const saved = await prisma.atsAnalysis.upsert({
      ...
      create: {
        ...
        experienceAlignment,
        titleAlignment,
        requiredSkills: requiredMatches as any,
        preferredSkills: preferredMatches as any,
        recommendations,
      },
      update: {
        overallCoverage,
