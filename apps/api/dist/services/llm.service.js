...
class LlmService {
     ...
     */
    static async analyzeScreeningQuestions(questions) {
        if (!groq)
            throw new Error('Groq SDK not configured (Missing GROQ_API_KEY).');
...
`;
     ...
     */
    static async generateTailoredCv(jobDescription, isLaravel, isNode) {
        if (!groq)
            throw new Error('Groq SDK not configured.');
...
`;
     ...
     */
    static async generateCoverLetter(jobDescription, companyName) {
        if (!groq)
            throw new Error('Groq SDK not configured.');
