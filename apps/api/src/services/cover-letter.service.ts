...
import { CoverLetterData } from '@ai-job-agent/shared';
...
export class CoverLetterService {
   ...
   */
  static async generateCoverLetter(jobId: string): Promise<CoverLetterData> {
    ...
    // ---------------------------------------------------------
    const clData = await AiService.generateCoverLetter(jobId, jobFull, job.company, profile, true);
  ...
  static async getCoverLettersByJob(jobId: string): Promise<CoverLetterData[]> {
  ...
  static async getCoverLetterById(id: string): Promise<{ coverLetter: CoverLetterData; job: any }> {
    const cl = await prisma.coverLetter.findUnique({
      where: { id },
      include: { job: true },
    });

    if (!cl) throw new Error(`CoverLetter not found: ${id}`);
  ...
  static async updateCoverLetter(id: string, bodyText: string): Promise<CoverLetterData> {
