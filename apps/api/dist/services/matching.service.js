...
class MatchingService {
    ...
    static async analyzeJob(jobId) {
        ...
        if (ai_service_1.AiService.isEnabled()) {
            try {
                const aiJson = await ai_service_1.AiService.analyzeJob(job.id, jobFullText, profile);
    ...
    static async saveAiMatchResult(job, aiJson) {
     ...
     */
    static async evaluateDeterministically(job, profile) {
        ...
        if (hasLaravel && hasPHP)
            technicalScore += 25;
        else if (hasLaravel || hasPHP)
            technicalScore += 18;
        if (hasNode)
            technicalScore += 10;
        if (hasVue || hasReact)
            technicalScore += 10;
        if (matchedSkills.includes('MySQL') || matchedSkills.includes('PostgreSQL'))
            technicalScore += 5;
        if (matchedSkills.includes('REST APIs'))
            technicalScore += 5;
        if (matchedSkills.includes('Docker'))
            technicalScore += 5;
        if (matchedSkills.includes('AWS'))
            technicalScore += 5;
        ...
        const overallMatch = Math.round(technicalScore * 0.5 + experienceScore * 0.25 + locationScore * 0.15 + (visaCompatibility === 'compatible' ? 10 : 5));
    ...
    static async callExternalLLM(job, profile) {
